...
public class KeyframeLineWidget extends GuiComponent implements Widget {
	...
	public KeyframeLineWidget(JGSelectionList pos, JGSelectionList rot, Font font, 
			JgModel model, AnimationScreen screen) {
		this.pos = pos;
		this.rot = rot;
		this.visible = true;
		this.minX = 10;
			...
			if(keyframes != null && model.getAnimation() != null) {
				...
				for(int i = 0; i < keyframes.size(); i++) {
					...
					if(kfx > minX-20 && kfx < maxVX+10) {
						...
						boolean animSelected = screen.getModel().getAnimator()
								.getCurrent()-1 == i;
						if(this.selected == i) {
	...
	public void processOnClick(int i) {
		try {
			...
			int j = 0;
			for(JgModelPart s : this.keyframes.get(i).translations.keySet()) {
			...
			int l = 0;
			for(JgModelPart s : this.keyframes.get(i).rotations.keySet()) {
