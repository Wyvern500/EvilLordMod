...
public class ModelPartsScreen extends Screen {
	...
	public ModelPartsScreen(JgModel gunModel) {
	...
	protected void init() {
		...
		//if(!init) {
			...
			JgPartKey[] gunParts = new JgPartKey[model.getPartsAsList().size()];
			for (int i = 0; i < model.getPartsAsList().size(); i++) {
				gunParts[i] = new JgPartKey(font, model.getPartsAsList().get(i));
			}
			gunPartList = new JGSelectionList(gunParts, this, this.font, 100, 2, 60, 14, 4, (k, i) -> {
				edits.get(0).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().pos[0]));
				edits.get(1).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().pos[1]));
				edits.get(2).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().pos[2]));
				edits.get(3).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().rot[0]));
				edits.get(4).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().rot[1]));
				edits.get(5).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().rot[2]));
			});
			...
			buttons.add(new Button(170, 2, 30, 20, new TranslatableComponent("Copy"), (b) -> {
				...
				if(indexes.size() == 1) {
					if(!booleanCycles.get(0).getValue()) {
						this.t = model.getPartsAsList().get(indexes.get(0)).getTransform().copy();
					} else {
						this.t = model.getPartsAsList().get(indexes.get(0)).getDTransform().copy();
					}
				}
			}));
			...
			buttons.add(new Button(170, 24, 30, 20, new TranslatableComponent("Paste"), (b) -> {
				...
				if(indexes.size() == 1 && this.t != null) {
					int i = indexes.get(0);
					if(!booleanCycles.get(0).getValue()) {
						model.getPartsAsList().get(i).getTransform().copyFrom(this.t);
						edits.get(0).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().pos[0]));
						edits.get(1).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().pos[1]));
						edits.get(2).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().pos[2]));
						edits.get(3).setValue(String.valueOf(model.getPartsAsList().get(i).getTransform().rot[0]));
						...
						model.getPartsAsList().get(i).getDTransform().copyFrom(this.t);
						edits.get(0).setValue(String.valueOf(model.getPartsAsList().get(i).getDTransform().pos[0]));
						edits.get(1).setValue(String.valueOf(model.getPartsAsList().get(i).getDTransform().pos[1]));
						edits.get(2).setValue(String.valueOf(model.getPartsAsList().get(i).getDTransform().pos[2]));
						edits.get(3).setValue(String.valueOf(model.getPartsAsList().get(i).getDTransform().rot[0]));
						edits.get(4).setValue(String.valueOf(model.getPartsAsList().get(i).getDTransform().rot[1]));
						edits.get(5).setValue(String.valueOf(model.getPartsAsList().get(i).getDTransform().rot[2]));
					}
				}
			}));
	...
	public Transform[] getTransforms() {
		...
		for (int i = 0; i < indexes.size(); i++) {
			...
			JgPartKey gunKey = ((JgPartKey) gunPartList.getKeys().get(ind));
	...
	public ModelPartsScreen setAnimScreen() {
