...
public class Animator {
	...
	protected JgModel model;
	
	protected Map<JgModelPart, float[]> currentTranslations;
	protected Map<JgModelPart, float[]> currentRotations;
	protected Map<JgModelPart, float[]> prevTranslations;
	protected Map<JgModelPart, float[]> prevRotations;
	
	public Animator(JgModel model) {
		this.model = model;
		this.old = -1;
		...
		for(int i = 0; i < model.getPartsAsList().size(); i++){
            this.prevTranslations.put(model.getPartsAsList().get(i), 
            		new float[] { 0, 0, 0 });
            this.prevRotations.put(model.getPartsAsList().get(i), 
                new float[] { 0, 0, 0 });
        }
		this.animation = null;
	}
	...
	public void update() {
		if(this.animation != null && model.canPlayAnimation()){
	            ...
	            doEasing();
	            for(JgModelPart part : currentTranslations.keySet()) {
	            ...
	            for(JgModelPart part : currentRotations.keySet()) {
	            	if(!prevRotations.containsKey(part)) {
	            		prevRotations.put(part, new float[] { 0, 0, 0 });
	            	}
	            	try {
	            	part.getTransform().rot[0] = Mth.rotLerp(this.prog, this.prevRotations
	            			.get(part)[0], this.currentRotations.get(part)[0]);
	            	part.getTransform().rot[1] = Mth.rotLerp(this.prog, this.prevRotations
	            			.get(part)[1], this.currentRotations.get(part)[1]);
	            	part.getTransform().rot[2] = Mth.rotLerp(this.prog, this.prevRotations
	...
	public void nextTick() {
        ...
        for(JgModelPart part : currentRotations.keySet()) {
        	...
        	try {
        	part.getTransform().rot[0] = Mth.rotLerp(this.prog, part1[0], 
        			part2[0]);
        	part.getTransform().rot[1] = Mth.rotLerp(this.prog, this.prevRotations
        			.get(part)[1], this.currentRotations.get(part)[1]);
        	part.getTransform().rot[2] = Mth.rotLerp(this.prog, this.prevRotations
	...
	public void prevTick() {
        ...
        for(JgModelPart part : currentRotations.keySet()) {
        	...
        	float[] part2 = this.currentRotations.get(part);
        	try {
        		part.getTransform().rot[0] = Mth.rotLerp(prog, part1[0], 
            			part2[0]);
            	part.getTransform().rot[1] = Mth.rotLerp(prog, part1[1], part2[1]);
            	part.getTransform().rot[2] = Mth.rotLerp(prog, part1[2], part2[2]);
        	} catch (NullPointerException e) {
				e.printStackTrace();
			}
        }
        if(this.tick-kf.startTick <= -1){
        	...
        	if(this.current <= 0) {
        		...
        		this.prevTranslations = new HashMap<>();
        		this.prevRotations = new HashMap<>();
        		List<JgModelPart> list = model.getPartsAsList();
        		for(int i = 0; i < list.size(); i++){
                    this.prevTranslations.put(list.get(i), 
                    		new float[] { 0, 0, 0 });
                    this.prevRotations.put(list.get(i), 
                        new float[] { 0, 0, 0 });
                }
                this.currentTranslations = this.animation.keyframes.get(current)
                    .translations;
	...
	public Map<JgModelPart, float[]> getCurrentTranslations() {
		return currentTranslations;
	}

	public void setCurrentTranslations(Map<JgModelPart, float[]> currentTranslations) {
		this.currentTranslations = currentTranslations;
	}

	public Map<JgModelPart, float[]> getCurrentRotations() {
		return currentRotations;
	}

	public void setCurrentRotations(Map<JgModelPart, float[]> currentRotations) {
		this.currentRotations = currentRotations;
	}

	public Map<JgModelPart, float[]> getPrevTranslations() {
		return prevTranslations;
	}

	public void setPrevTranslations(Map<JgModelPart, float[]> prevTranslations) {
		this.prevTranslations = prevTranslations;
	}

	public Map<JgModelPart, float[]> getPrevRotations() {
		return prevRotations;
	}

	public void setPrevRotations(Map<JgModelPart, float[]> prevRotations) {
